...
type ConversationTurn struct {
	...
	AgentCalls      []string `json:"agent_calls"`
...
func (dp *DataProvider) expandPath(p string) string {
	if strings.HasPrefix(p, "~") {
		...
		if err == nil { return filepath.Join(home, p[1:]) }
	}
	return p
}
...
func (dp *DataProvider) normalizeOpenClawPath(p string) string {
	...
	if strings.Contains(p, ".openclaw") {
		...
		if len(parts) == 2 {
			...
			if _, err := os.Stat(newPath); err == nil { return newPath }
		}
	}
	return p
}
...
func (dp *DataProvider) WatchRawStream(broadcaster *Broadcaster) {
	...
	go func() {
		defer watcher.Close()
		var lastSize int64
		if info, err := os.Stat(logPath); err == nil { lastSize = info.Size() }
		for {
			select {
	...
	for scanner.Scan() {
		...
		if agentID != "" {
			...
			if entry.Delta != "" {
				...
				broadcaster.Broadcast(Event{Type: "reasoning_delta", Payload: map[string]string{"agent_id": agentID, "delta": entry.Delta}})
			}
			dp.mu.Unlock()
...
func (dp *DataProvider) GetSessionDetails(agentID string, sessionID string) (*SessionDetails, error) {
	...
	for scanner.Scan() {
		...
		if line.Message.Role == "user" {
			...
			// Estimate injected context for this agent
			...
			if strings.HasPrefix(msg, "A new session was started") || 
			   strings.HasPrefix(msg, "HEARTBEAT") || 
			   strings.HasPrefix(msg, "System: [") ||
			   strings.Contains(msg, "configured persona") ||
			   strings.Contains(msg, "An async command you ran earlier has completed") {
				currentTurn.UserSource = "system"
			} else if strings.HasPrefix(msg, "[") {
				currentTurn.UserSource = "user" // Likely Telegram/Channel
			}
			...
			currentTurn.CacheReadTokens += line.Message.Usage.CacheRead
			currentTurn.Cost += line.Message.Usage.Cost.Total
			
			// Update session totals
			details.TotalTokens += (line.Message.Usage.Input + line.Message.Usage.Output)
			details.TotalCacheRead += line.Message.Usage.CacheRead
			details.TotalCost += line.Message.Usage.Cost.Total

			currentTurn.Timestamp = line.Timestamp
				...
				for _, part := range contentArr {
					if p, ok := part.(map[string]interface{}); ok {
						if p["type"] == "thinking" { reasoningBuilder.WriteString(p["thinking"].(string))
							...
							if name == "sessions_send" || name == "coding-agent" || name == "sessions_spawn" {
								if input, ok := p["input"].(map[string]interface{}); ok {
									if targetID, ok := input["agentId"].(string); ok {
										currentTurn.AgentCalls = append(currentTurn.AgentCalls, targetID)
									}
								}
							}
						}
	...
	if currentTurn != nil {
		currentTurn.Reasoning = reasoningBuilder.String()
		dp.mu.RLock()
		if dp.agentStatus[agentID] == "thinking" && dp.agentReasoning[agentID] != "" { currentTurn.Reasoning = dp.agentReasoning[agentID] }
		dp.mu.RUnlock()
		details.Turns = append(details.Turns, *currentTurn)
	}
	return details, nil
...
func (dp *DataProvider) estimateContextStats(agentID string) ([]string, int64) {
	// Standard OpenClaw context files
...
func extractThinkingFromTaggedText(text string) string {
	...
	for _, t := range tags {
		...
		if start != -1 && end != -1 && end > start { return text[start+len(t[0]) : end] }
	}
	return ""
}
...
func (dp *DataProvider) GetAgentPersonas() ([]AgentPersona, error) {
	...
	for _, s := range skills { externalSkills[s.Name] = true }
	allCronJobs, _ := dp.GetCronJobs()
	...
	var cfg struct {
		Agents struct {
			List []struct {
				...
				Skills    []string `json:"skills"`
				Model     struct { Primary string `json:"primary"` } `json:"model"`
				Identity  struct { Emoji string `json:"emoji"` } `json:"identity"`
				Tools     struct { Profile string `json:"profile"` } `json:"tools"`
			} `json:"list"`
		} `json:"agents"`
	}
	...
	for idx, a := range cfg.Agents.List {
		...
		if agentDir == "" { agentDir = filepath.Join(dp.BasePath, "agents", id, "agent") }
		sessionsDir := dp.normalizeOpenClawPath(filepath.Join(dp.BasePath, "agents", id, "sessions"))
		name := a.Name
		if name == "" { name = strings.Title(id) }
		emoji := a.Identity.Emoji
		if emoji == "" { emoji = dp.readEmojiFromIdentity(workspace) }
		if emoji == "" {
			lid := strings.ToLower(id)
			if strings.Contains(lid, "coder") { emoji = "💻"
			} else if strings.Contains(lid, "research") { emoji = "🔍"
			} else if strings.Contains(lid, "schedule") { emoji = "📅"
			} else { emoji = "🧠" }
		}
		var filteredSkills []string
		if a.Skills != nil {
			for _, s := range a.Skills {
				if externalSkills[s] { filteredSkills = append(filteredSkills, s) }
			}
		} else { filteredSkills = dp.getSkillNames(externalSkills) }
		dp.mu.RLock()
		status := dp.agentStatus[id]
		if status == "" { status = "idle" }
		lastActive := dp.lastUpdate[id].Unix()
		dp.mu.RUnlock()
		var agentCrons []CronJob
		for _, c := range allCronJobs { if c.AgentID == id { agentCrons = append(agentCrons, c) } }
		snippet := dp.readCapabilitiesSnippet(workspace)
		agentRoot := filepath.Dir(workspace)
		isDefault := a.Default
		if idx == 0 { isDefault = true }
		personas = append(personas, AgentPersona{
			ID: id, Name: name, Emoji: emoji, Workspace: workspace, AgentDir: agentRoot,
			Sessions: sessionsDir, IsDefault: isDefault, Skills: filteredSkills,
			Status: status, LastActive: lastActive, Model: a.Model.Primary,
			ToolProfile: a.Tools.Profile, CronJobs: agentCrons, Snippet: snippet,
		})
	}
	return personas, nil
}
...
func (dp *DataProvider) GetAgentPersonasInternal() ([]AgentPersona, error) {
	...
	for _, a := range cfg.Agents.List {
		workspace := dp.normalizeOpenClawPath(a.Workspace)
		if workspace == "" {
			if a.ID == "main" { workspace = filepath.Join(dp.BasePath, "workspace")
			} else { workspace = filepath.Join(dp.BasePath, "workspace-"+a.ID) }
		}
...
func (dp *DataProvider) readCapabilitiesSnippet(workspace string) string {
	...
	for _, line := range lines {
		trimmed := strings.TrimSpace(line)
		if strings.Contains(trimmed, "## Capabilities") { found = true; continue }
		if found {
			...
			if trimmed != "" {
				...
				for {
					changed := false
					if strings.HasPrefix(text, "-") { text = text[1:]; changed = true }
					if strings.HasPrefix(text, "*") { text = text[1:]; changed = true }
					if strings.HasPrefix(text, " ") { text = text[1:]; changed = true }
					if !changed { break }
				}
				if text != "" {
					...
					if snippetBuilder.Len() > 0 { snippetBuilder.WriteString(" ") }
					snippetBuilder.WriteString(text)
					if snippetBuilder.Len() > 150 { break }
				}
			}
		}
	}
	runes := []rune(snippetBuilder.String())
	if len(runes) > 50 { return string(runes[:50]) + "..." }
	return string(runes)
}
...
func (dp *DataProvider) GetSubAgentActivity() ([]SubAgentRun, error) {
	...
	if data, err := os.ReadFile(legacyPath); err == nil {
		...
		if err := json.Unmarshal(data, &runs); err == nil { allRuns = append(allRuns, runs...) }
	}
	...
	if data, err := os.ReadFile(path); err == nil {
		...
		if err := json.Unmarshal(data, &registry); err != nil {
			for _, record := range registry.Runs {
				name := record.Label
				if name == "" { name = record.Task }
				status := "running"
				if record.Outcome != nil {
					if record.Outcome.Status == "ok" { status = "completed"
					} else if record.Outcome.Status == "error" { status = "failed" }
				}
				duration := "--"
				if record.StartedAt > 0 {
					var d time.Duration
					if record.EndedAt > 0 { d = time.Duration(record.EndedAt-record.StartedAt) * time.Millisecond
					} else { d = time.Since(time.Unix(0, record.StartedAt*int64(time.Millisecond))) }
					duration = d.Round(time.Second).String()
				}
				allRuns = append(allRuns, SubAgentRun{ID: record.RunID, Name: name, Status: status, Duration: duration})
			}
		}
	}
	return allRuns, nil
...
func (dp *DataProvider) WatchTranscripts(broadcaster *Broadcaster) error {
	...
	for _, p := range personas {
		if _, err := os.Stat(p.Sessions); err == nil { _ = watcher.Add(p.Sessions) }
	}
	go func() {
		defer watcher.Close()
		for {
			...
			case event, ok := <-watcher.Events:
				if !ok { return }
				if (event.Op&fsnotify.Write == fsnotify.Write || event.Op&fsnotify.Create == fsnotify.Create) && filepath.Ext(event.Name) == ".jsonl" {
...
func (dp *DataProvider) handleTranscriptChange(path string, broadcaster *Broadcaster) {
	...
	defer file.Close()
	stat, _ := file.Stat()
	if stat.Size() < 2000 { _, _ = file.Seek(0, 0)
	} else { _, _ = file.Seek(-2000, 2) }
	scanner := bufio.NewScanner(file)
	var lastLine string
	...
	var entry struct {
		Message struct {
			Role    string `json:"role"`
			Content []struct { Type string `json:"type"`; Name string `json:"name"` } `json:"content"`
			ToolCalls []struct { Function struct { Name string `json:"name"` } `json:"function"` } `json:"tool_calls"`
		} `json:"message"`
	}
	...
	for _, c := range entry.Message.Content {
		if c.Type == "tool_use" || c.Type == "tool_call" {
			broadcaster.Broadcast(Event{Type: "skill_use", Payload: map[string]string{"agent_id": agentID, "skill": c.Name, "status": "executing"}})
			return
		}
	}
	for _, tc := range entry.Message.ToolCalls {
		if tc.Function.Name != "" {
			broadcaster.Broadcast(Event{Type: "skill_use", Payload: map[string]string{"agent_id": agentID, "skill": tc.Function.Name, "status": "executing"}})
			return
		}
	}
...
func (dp *DataProvider) ListFiles() ([]string, error) {
	var files []string
	_ = filepath.Walk(dp.BasePath, func(path string, info os.FileInfo, err error) error {
...
func (dp *DataProvider) GetAlerts() ([]Alert, error) {
	...
	start := 0
	if len(lines) > 5 { start = len(lines) - 5 }
	for _, line := range lines[start:] {
		parts := strings.SplitN(line, " - ", 2)
		if len(parts) == 2 {
			meta := strings.Fields(parts[0])
			if len(meta) >= 2 {
				alerts = append(alerts, Alert{Level: strings.Trim(meta[0], "[]"), Time: strings.Join(meta[1:], " "), Message: parts[1]})
			}
		}
	}
	return alerts, nil
...
func (dp *DataProvider) GetGitLog() ([]GitCommit, error) {
	...
	if err != nil { return []GitCommit{}, nil }
...
func (dp *DataProvider) GetCronJobs() ([]CronJob, error) {
	path := filepath.Join(dp.BasePath, "cron", "jobs.json")
	data, err := os.ReadFile(path)
	if err != nil { return []CronJob{}, nil }
	var raw struct {
		Jobs []struct {
			ID string `json:"id"`; AgentID string `json:"agentId"`; Name string `json:"name"`; Desc string `json:"description"`; Enabled bool `json:"enabled"`
			Schedule struct { Kind string `json:"kind"`; Expr string `json:"expr"` } `json:"schedule"`
			State struct { NextRunAtMs int64 `json:"nextRunAtMs"` } `json:"state"`
		} `json:"jobs"`
	}
	...
	for _, j := range raw.Jobs {
		nextRun := ""
		if j.State.NextRunAtMs > 0 { nextRun = time.Unix(j.State.NextRunAtMs/1000, 0).Format("2006-01-02 15:04") }
		jobs = append(jobs, CronJob{ID: j.ID, AgentID: j.AgentID, Name: j.Name, Description: j.Desc, Enabled: j.Enabled, Schedule: j.Schedule.Expr, NextRun: nextRun})
	}
	return jobs, nil
}
...
func (dp *DataProvider) GetModels() ([]Model, error) {
	...
	if data, err := os.ReadFile(path); err == nil {
		var models []Model
		if err := json.Unmarshal(data, &models); err != nil { return models, nil }
	}
	configPath := filepath.Join(dp.BasePath, "openclaw.json")
	if data, err := os.ReadFile(configPath); err == nil {
		var cfg struct { Agents struct { Defaults struct { Models map[string]interface{} `json:"models"` } `json:"defaults"` } `json:"agents"` }
		if err := json.Unmarshal(data, &cfg); err != nil { return nil, err }
		var models []Model
		for id := range cfg.Agents.Defaults.Models { models = append(models, Model{Name: id, Type: "chat"}) }
		return models, nil
	}
	return []Model{}, nil
}
...
func (dp *DataProvider) GetSkills() ([]Skill, error) {
	...
	if data, err := os.ReadFile(configPath); err == nil {
		var cfg struct { Skills struct { Load struct { ExtraDirs []string `json:"extraDirs"` } `json:"load"` } `json:"skills"` }
		if err := json.Unmarshal(data, &cfg); err != nil { extraDirs = cfg.Skills.Load.ExtraDirs }
	}
	...
	for _, e := range entries {
		...
		if _, err := os.Stat(skillPath); err == nil {
			...
			for scanner.Scan() {
				...
				if inFrontmatter {
					if strings.HasPrefix(line, "name:") { name = strings.TrimSpace(strings.TrimPrefix(line, "name:"))
...
type UsageBucket struct {
	Calls int `json:"calls"`; Input int64 `json:"input"`; Output int64 `json:"output"`; CacheRead int64 `json:"cache_read"`; TotalTokens int64 `json:"total_tokens"`; Cost float64 `json:"cost"`
}

func FriendlyModelName(model string) string {
	ml := strings.ToLower(model)
	if strings.Contains(ml, "opus-4-6") { return "Claude Opus 4.6"
	} else if strings.Contains(ml, "opus") { return "Claude Opus 4.5"
	} else if strings.Contains(ml, "sonnet") { return "Claude Sonnet"
	} else if strings.Contains(ml, "haiku") { return "Claude Haiku"
	} else if strings.Contains(ml, "gpt-5") { return "GPT-5"
	} else if strings.Contains(ml, "gpt-4o") { return "GPT-4o"
	} else if strings.Contains(ml, "gpt-4") { return "GPT-4"
	} else if strings.Contains(ml, "gemini") { return "Gemini" }
	return model
}
...
func (dp *DataProvider) GetDetailedUsage() (map[string]UsageBucket, error) {
	...
	for _, f := range files {
		...
		for scanner.Scan() {
			...
			if line.Message.Role != "assistant" || line.Message.Model == "" { continue }
			name := FriendlyModelName(line.Message.Model)
			bucket := usage[name]
			bucket.Calls++; bucket.Input += line.Message.Usage.Input; bucket.Output += line.Message.Usage.Output; bucket.CacheRead += line.Message.Usage.CacheRead; bucket.TotalTokens += line.Message.Usage.TotalTokens; bucket.Cost += line.Message.Usage.Cost.Total
			usage[name] = bucket
		}
		file.Close()
	}
...
func (dp *DataProvider) GetGeneratedAlerts() ([]Alert, error) {
	...
	if err == nil {
		...
		if totalRSS > 512000 { alerts = append(alerts, Alert{Level: "WARNING", Message: fmt.Sprintf("High system memory usage: %.0f MB", totalRSS/1024), Time: time.Now().Format("15:04:05")}) }
	}
	usage, _ := dp.GetDetailedUsage()
	totalCost := 0.0
	for _, b := range usage { totalCost += b.Cost }
	if totalCost > 20.0 { alerts = append(alerts, Alert{Level: "WARNING", Message: fmt.Sprintf("Daily cost exceeds $20: $%.2f", totalCost), Time: time.Now().Format("15:04:05")}) }
	return alerts, nil
}
